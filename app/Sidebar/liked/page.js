    ...
    <ProtectedRoute>
      <Layout>
        ...
        <div className='flex mt-24 flex-col md:flex-col lg:flex-row max-w-6xl mx-auto justify-evenly px-3'>
          <div className='mt-7'>
            <h2 className='text-sm text-white md:text-lg lg:text-xl xl:text-2xl 2xl:text-2xl font-serif'>Liked Total Video: {like?.length}</h2></div>
          <div>
            {
              like &&
              like.map((data, index) => (
                        ...
                        <div className="flex">
                          ...
                          <div className="w-full px-8 py-4">
                            <div className="flex justify-between">
                              <Link href={`/movies/${data?.data._id}`}>
                                <div>
                                  <h3 className="text-base md:text-xl text-white lg:text-2xl hover:text-green-500 text-grey-darkest font-medium">{data?.data?.title} </h3>
                                  <p className="text-sm text-grey text-white mt-1">{data?.data?.publisDate}</p>
                                </div>
                              </Link>
